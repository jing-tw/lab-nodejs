...
class RTCAudioSourceWAVFile {
  constructor(options = {}) {
    ...
    // method 1: debug
    byteBuffer = byteBuffer.slice(44); // skip wave header
    let bytesPerFrame = numberOfFrames * channelCount * bitsPerSample/8;
    let offset = 0;
    function next() {
      for (let i = 0; i < numberOfFrames; i++) {
        samples[i] = byteBuffer.readInt16LE(i*2 + offset);
      }
